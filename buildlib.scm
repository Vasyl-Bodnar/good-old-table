  ...
  (when conditional
    ...
    (let* ((metadata (in-vicinity *obj-build-directory* "metafile"))
      ...
      (when (and *library* (memq *library-type* '(static dynamic both)))
        ...
        (if (and (check-fail)
                 (or (and (memq *library-type* '(static both)) (not (file-exists? (string-append *library* ".a"))))
                     (and (memq *library-type* '(dynamic both)) (not (file-exists? (string-append *library* ".so"))))
      ...
      (if (and (check-fail)
          ...
          (when (not hashed-exes) (info "Already hashed all sources")))))
  (return-fail))
